...
@php
    $wishlistCount = auth()->check() && auth()->user()->role !== 'admin'
        ...
        : 0;
@endphp
<nav id="navbar" data-transparent="{{ ($transparent ?? false) ? 'true' : 'false' }}"
     class="fixed top-0 left-0 right-0 z-50 transition-all duration-300 {{ ($transparent ?? false) ? 'bg-transparent' : 'bg-white shadow-lg shadow-black/5' }}">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 group">
                ...
                <span id="logo-text" class="text-base md:text-xl font-bold transition-colors {{ ($transparent ?? false) ? 'text-white' : 'text-dark-900' }}">
                ...
                @foreach($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                       class="nav-link-hero px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 hover:bg-primary-50 hover:text-primary-500
                              {{ ($transparent ?? false) ? 'text-white' : 'text-dark-900' }}
            ...
            <div class="hidden md:flex items-center justify-end gap-3 min-w-[180px]">
                @auth
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}"
                           class="nav-link-hero px-4 py-2 rounded-full text-sm font-medium transition-all {{ ($transparent ?? false) ? 'text-white' : 'text-dark-900' }} hover:text-primary-500">
                            Admin
                        </a>
                    ...
                    @else
                        <details class="relative group">
                            <summary class="list-none cursor-pointer flex items-center gap-2 rounded-full pl-2 pr-4 py-2 transition-all {{ ($transparent ?? false) ? 'text-white hover:bg-white/10' : 'text-dark-900 hover:bg-gray-100' }}">
                ...
                @else
                    <a href="{{ route('login') }}"
                       class="nav-link-hero px-5 py-2.5 rounded-full text-sm font-medium transition-all {{ ($transparent ?? false) ? 'text-white hover:bg-white/10' : 'text-dark-900 hover:bg-gray-100' }}">
                        Login
                    </a>
            ...
            <div class="md:hidden flex items-center gap-2">
                @auth
                    @if(auth()->user()->role !== 'admin')
                        <a href="{{ route('user.wishlist') }}"
                           aria-label="Wishlist"
                           class="js-wishlist-nav-target nav-link-hero relative w-10 h-10 rounded-full flex items-center justify-center transition-colors {{ ($transparent ?? false) ? 'text-white hover:bg-white/10' : 'text-dark-900 hover:bg-gray-100' }}">
                ...
                @else
                    <a href="{{ route('login') }}"
                       aria-label="Wishlist"
                       class="nav-link-hero w-10 h-10 rounded-full flex items-center justify-center transition-colors {{ ($transparent ?? false) ? 'text-white hover:bg-white/10' : 'text-dark-900 hover:bg-gray-100' }}">
                ...
                <button id="mobile-menu-btn" aria-label="Buka menu"
                        class="nav-link-hero w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 {{ ($transparent ?? false) ? 'text-white hover:bg-white/10' : 'text-dark-900 hover:bg-gray-100' }}">
        ...
        {{-- Mobile Menu --}}
        <div id="mobile-menu" class="hidden absolute top-[calc(100%+1rem)] right-4 left-4 md:hidden z-[100] bg-white rounded-3xl shadow-2xl ring-1 ring-black/5 overflow-hidden">
